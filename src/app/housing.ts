...
export class HousingService{
    protected housingLocationList: HousinglocationInfo[] = [
      ...
      addHousingLocation(location: HousinglocationInfo): void {
        this.housingLocationList.push(location);
      }
    
      updateHousingLocation(updatedLocation: HousinglocationInfo): void {
        const index = this.housingLocationList.findIndex(location => location.id === updatedLocation.id);
        if (index !== -1) {
          this.housingLocationList[index] = updatedLocation;
        }
      }
